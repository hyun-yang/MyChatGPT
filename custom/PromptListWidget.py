...
class PromptListWidget(QWidget):
    ...
    def __init__(self, table_name=Constants.CHAT_PROMPT_TABLE, db_connection_name=None):
        super().__init__()
        self.db_connection_name = db_connection_name or f"PromptDBConnection_{uuid.uuid4().hex[:8]}"
        self.table_name = table_name
        self.initialize_ui()

    def initialize_ui(self):
        if QSqlDatabase.contains(self.db_connection_name):
            QSqlDatabase.removeDatabase(self.db_connection_name)

        self._promptDB = QSqlDatabase.addDatabase('QSQLITE', self.db_connection_name)
        self._promptDB.setDatabaseName(Constants.DATABASE_NAME)

        if not self._promptDB.open():
            QMessageBox.critical(self, "Database Error",
                                 f"Failed to open database: {self._promptDB.lastError().text()}")
            return

        self.model = PromptLazyLoadTableModel(self._promptDB, table_name=self.table_name)
        ...
        self.promptListView.setColumnHidden(0, True)
    ...
    def closeEvent(self, event):
        if hasattr(self, '_promptDB') and self._promptDB.isOpen():
            self._promptDB.close()
        if QSqlDatabase.contains(self.db_connection_name):
            QSqlDatabase.removeDatabase(self.db_connection_name)
...
class PromptLazyLoadTableModel(QAbstractTableModel):
    ...
    def __init__(self, db_connection, table_name=Constants.CHAT_PROMPT_TABLE):
        super().__init__()
    ...
    def load_initial_data(self):
        ...
        if query.exec(f"SELECT id, title, prompt FROM {self.table_name}"):
            while query.next():
                id = query.value(0)
                title = query.value(1)
            ...
            QMessageBox.critical(None, DATABASE_MESSAGE.DATABASE_TITLE_ERROR,
                                 f"Failed to load data: {error.text()}")
    ...
    def reload_data(self):
        ...
        if query.exec(f"SELECT id, title, prompt FROM {self.table_name}"):
            ...
            error = query.lastError()
            QMessageBox.critical(None, DATABASE_MESSAGE.DATABASE_TITLE_ERROR,
                                 f"Failed to reload data: {error.text()}")
        self.endResetModel()
    ...
    def fetch_prompt(self, row):
        ...
        queryData = {}
        query.prepare(f"SELECT title, prompt FROM {self.table_name} WHERE id = ?")
    ...
    def delete_row(self, row):
        if not self.db_connection.isOpen() or row >= len(self.data_cache):
            return

        id = self.data_cache[row][0]
        query = QSqlQuery(self.db_connection)
        query.prepare(f"DELETE FROM {self.table_name} WHERE id = ?")
        query.addBindValue(id)
        if query.exec():
    ...
    def add_row(self):
        ...
        query = QSqlQuery(self.db_connection)
        query.prepare(f"INSERT INTO {self.table_name} (title, prompt) VALUES (?, ?)")
    ...
    def update_row(self, row, title, prompt):
        ...
        id = self.data_cache[row][0]
        query = QSqlQuery(self.db_connection)
        query.prepare(f"UPDATE {self.table_name} SET title = ?, prompt = ? WHERE id = ?")
