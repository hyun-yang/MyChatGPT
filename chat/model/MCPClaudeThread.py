...
class MCPClaudeThread(QThread):
                ...
                for content in response.content:
                    ...
                    if content.type == 'text':
                        ...
                        if (len(response.content) == 1 and response.content[0].type == "text"):
                            ...
                            self.response_signal.emit(f"{response.content[0].text}\n", self.stream)
                            self.finish_run()
                            processing = False
    ...
    def _cleanup_loop(self):
        ...
        try:
            ...
            if pending:
                print(f"MCPClaudeThread: Cancelling {len(pending)} pending tasks...")
                for task in pending:
                    if not task.done():
                        task.cancel()
                ...
                try:
                    ...
                    print("MCPClaudeThread: Task cleanup timed out")
                except Exception as e:
                    print(f"MCPClaudeThread: Error during task cleanup: {e}")
            ...
            if not self.loop.is_closed():
                self.loop.close()
                print("MCPClaudeThread: Event loop closed")

        except Exception as e:
            print(f"MCPClaudeThread: Error during loop cleanup: {e}")
        finally:
            self.loop = None

    async def _cleanup_resources(self):
        try:
            print("MCPClaudeThread: Cleaning up resources...")
            ...
            print("MCPClaudeThread: Resources cleaned up successfully")
        except Exception as e:
            print(f"MCPClaudeThread: Error during cleanup: {e}")
