...
class OrchestratorClaudeThread(QThread):
    ...
    def __init__(self, args):
        ...
        self.async_anthropic = anthropic.AsyncAnthropic(api_key=args['api_key'])
    ...
    async def _async_run(self):
        try:
            ...
            if self.stream:
                ...
                full_response_not_stream = response.content[0].text
            ...
            try:
                ...
                if self.stream:
                    ...
                    ai_args['messages'] = [{"role": "user", "content": aggregator_prompt}]
                    ...
                    final_full_response = final_response.content[0].text
    ...
    async def get_response(self, claude_arg):
        ai_args = self._prepare_claude_args(claude_arg)
        stream = await self.async_anthropic.messages.create(**ai_args)
        return stream
    ...
    async def get_async_response(self, claude_arg):
        ai_args = self._prepare_claude_args(claude_arg)
        ai_args.pop('stream', None)

        response = await self.async_anthropic.messages.create(**ai_args)
        return response

    def _prepare_claude_args(self, claude_arg):
        ai_args = claude_arg.copy()

        if 'messages' not in ai_args:
            ai_args['messages'] = [{"role": "user", "content": self.orchestrator_prompt}]

        if 'max_tokens' not in ai_args:
            ai_args['max_tokens'] = 4000

        if 'model' not in ai_args or not ai_args['model'].startswith('claude'):
            ai_args['model'] = 'claude-3-5-sonnet-20241022'
    ...
    async def call_llm_async(self, prompt: str, ai_arg: dict) -> str:
        try:
            ai_args = self._prepare_claude_args(ai_arg)
            ai_args['messages'] = [{"role": "user", "content": prompt}]
            ai_args.pop('stream', None)

            chat_completion = await self.async_anthropic.messages.create(**ai_args)
