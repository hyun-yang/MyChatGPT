...
class OrchestratorOllamaThread(QThread):
    ...
    async def _async_run(self):
        try:
            ...
            if self.stream:
                ...
                full_response_not_stream = response['message']['content']
            ...
            try:
                ...
                if self.stream:
                    ...
                    messages = []
                    messages.append({"role": "user", "content": aggregator_prompt})
                    ...
                    final_full_response = final_response['message']['content']
    ...
    async def get_response(self, ollama_arg):
        ai_args = self.prepare_ollama_args(ollama_arg)
        ai_args['stream'] = True

        stream = await self.ollama_client.chat(**ai_args)
        return stream
    ...
    async def get_async_response(self, ollama_arg):
        ai_args = self.prepare_ollama_args(ollama_arg)
        ai_args['stream'] = False

        response = await self.ollama_client.chat(**ai_args)
        return response

    def prepare_ollama_args(self, ai_arg):
        ollama_args = {
            'model': ai_arg['model'],
            'messages': ai_arg['messages']
        }

        if 'temperature' in ai_arg:
            ollama_args['options'] = ollama_args.get('options', {})
            ollama_args['options']['temperature'] = ai_arg['temperature']

        if 'max_tokens' in ai_arg:
            ollama_args['options'] = ollama_args.get('options', {})
            ollama_args['options']['num_predict'] = ai_arg['max_tokens']

        if 'top_p' in ai_arg:
            ollama_args['options'] = ollama_args.get('options', {})
            ollama_args['options']['top_p'] = ai_arg['top_p']
    ...
    async def call_llm_async(self, prompt: str, ai_arg: dict) -> str:
        try:
            ...
            ai_args = self.prepare_ollama_args({**ai_arg, 'messages': messages})
            ai_args['stream'] = False
