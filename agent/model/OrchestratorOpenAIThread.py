...
class OrchestratorOpenAIThread(QThread):
    ...
    def __init__(self, args):
        ...
        self.async_openai = AsyncOpenAI(api_key=args['api_key'])
    ...
    async def _async_run(self):
        try:
            ...
            if self.stream:
                ...
                full_response_not_stream = response.choices[0].message.content
            ...
            try:
                ...
                if self.stream:
                    ...
                    final_full_response = final_response.choices[0].message.content
    ...
    async def get_response(self, openai_arg):
        ai_args = openai_arg.copy()
        stream = await self.async_openai.chat.completions.create(**ai_args)
        return stream
    ...
    async def get_async_response(self, openai_arg):
        ai_args = openai_arg.copy()
        if 'stream' in ai_args:
            ai_args.pop('stream')

        response = await self.async_openai.chat.completions.create(**ai_args)
        return response
    ...
    async def call_llm_async(self, prompt: str, ai_arg: dict) -> str:
        try:
            ...
            chat_completion = await self.async_openai.chat.completions.create(**ai_args)
