...
class EvaluatorClaudeThread(QThread):
    ...
    def get_response(self, prompt: str, system_prompt: str = "", model="claude-3-5-sonnet-20241022") -> str:
        if self.force_stop:
            self.finish_run(self.model, Constants.FORCE_STOP, self.stream)
            return ""
        ...
        if self.stream:
            return self.dispatch_response(messages, system_prompt)
        else:
    ...
    def dispatch_response(self, messages: list, system_prompt: str = "") -> str:
        if self.force_stop:
            self.finish_run(self.model, Constants.FORCE_STOP, self.stream)
            return ""

        response_stream = self.claude.messages.create(
            model=self.ai_arg['model'],
            max_tokens=self.ai_arg['max_tokens'],
            system=system_prompt,
            messages=messages,
