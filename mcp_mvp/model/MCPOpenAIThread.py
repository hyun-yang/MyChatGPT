...
class MCPOpenAIThread(QThread):
    ...
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.openai_client = openai.OpenAI(api_key=args['api_key'])
        self.ai_arg = args['ai_arg']
        self.model = self.ai_arg['model']
        self.max_tokens = self.ai_arg['max_tokens']
    ...
    async def process_query(self, ai_arg) -> None:
        ...
        try:
            ...
            while processing and not self.force_stop:
                # Call OpenAI ChatCompletion with functions
                ...
                # If function_call is present, handle tool use
    ...
    def _cleanup_loop(self):
        ...
        try:
            ...
            if pending:
                print(f"MCPOpenAIThread: Cancelling {len(pending)} pending tasks...")
                for task in pending:
                    if not task.done():
                        task.cancel()
                ...
                try:
                    ...
                    print("MCPOpenAIThread: Task cleanup timed out")
                except Exception as e:
                    print(f"MCPOpenAIThread: Error during task cleanup: {e}")
            ...
            if not self.loop.is_closed():
                self.loop.close()
                print("MCPOpenAIThread: Event loop closed")

        except Exception as e:
            print(f"MCPOpenAIThread: Error during loop cleanup: {e}")
        finally:
            self.loop = None

    async def _cleanup_resources(self):
        try:
            print("MCPOpenAIThread: Cleaning up resources...")
            ...
            print("MCPOpenAIThread: Resources cleaned up successfully")
        except Exception as e:
            print(f"MCPOpenAIThread: Error during cleanup: {e}")
