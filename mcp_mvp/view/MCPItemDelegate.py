...
class MCPItemDelegate(QStyledItemDelegate):
    row_id_signal = pyqtSignal(int)

    def __init__(self, mcp_view, parent=None):
        super().__init__(parent)
        self.mcp_view = mcp_view
    ...
    def editorEvent(self, event, model, option, index):
        if event.type() == QMouseEvent.Type.MouseButtonPress:
            if QRect(option.rect.right() - UI.ITEM_ICON_SIZE * 2 - UI.ITEM_PADDING, option.rect.top(),
                ...
                mcp_item = model.get_mcp(index)
                self.mcp_view.rename_chat(index, mcp_item['title'])
